...
export const PiutangAktif: React.FC = () => {
  ...
  // Filters state
  const [searchQuery, setSearchQuery] = useState(() => getStoredState('piutang_searchQuery', ''));
  const [statusFilter, setStatusFilter] = useState<'all' | 'lancar' | 'overdue'>(() => getStoredState('piutang_statusFilter', 'all'));
  ...
  // Keyboard navigation
  const [selectedCustIdx, setSelectedCustIdx] = useState<number>(() => getStoredState('piutang_selectedCustIdx', 0));
  const [selectedInvoiceIdx, setSelectedInvoiceIdx] = useState<number | null>(() => getStoredState('piutang_selectedInvoiceIdx', null));
  ...
  const [showPaymentModal, setShowPaymentModal] = useState(() => getStoredState('piutang_showPaymentModal', false));
  const [paymentMode, setPaymentMode] = useState<'fifo' | 'manual'>(() => getStoredState('piutang_paymentMode', 'fifo'));
  const [paymentAmount, setPaymentAmount] = useState<number | ''>(() => getStoredState('piutang_paymentAmount', ''));
  const [manualAmounts, setManualAmounts] = useState<Record<string, number>>(() => getStoredState('piutang_manualAmounts', {}));
  const [paymentMethod, setPaymentMethod] = useState(() => getStoredState('piutang_paymentMethod', 'cash'));
  const [paymentNote, setPaymentNote] = useState(() => getStoredState('piutang_paymentNote', ''));
  const [paymentDate, setPaymentDate] = useState(() => getStoredState('piutang_paymentDate', new Date().toISOString().slice(0, 10)));
  const [syncNotaOtomatis, setSyncNotaOtomatis] = useState(() => getStoredState('piutang_syncNotaOtomatis', true));

  // Modal invoice detail (F4)
  const [showDetailModal, setShowDetailModal] = useState(() => getStoredState('piutang_showDetailModal', false));
  const [detailInvoiceId, setDetailInvoiceId] = useState<string | null>(() => getStoredState('piutang_detailInvoiceId', null));
  const [detailInvoice, setDetailInvoice] = useState<any | null>(null);
  const [isLoadingDetail, setIsLoadingDetail] = useState(false);

  // Modal print receipt
  const [showReceiptModal, setShowReceiptModal] = useState(() => getStoredState('piutang_showReceiptModal', false));
  const [receiptSession, setReceiptSession] = useState<any | null>(() => getStoredState('piutang_receiptSession', null));

  // Modal print billing
  const [showBillingPrintModal, setShowBillingPrintModal] = useState(() => getStoredState('piutang_showBillingPrintModal', false));
  const [printBillingGroup, setPrintBillingGroup] = useState<CustomerGroup | null>(() => getStoredState('piutang_printBillingGroup', null));

  // Customer search selection popup modal
  const [showSearchPopup, setShowSearchPopup] = useState(false);
  const [isConfirmed, setIsConfirmed] = useState(() => getStoredState('piutang_isConfirmed', false));
  ...
  useEffect(() => {
    localStorage.setItem('piutang_searchQuery', JSON.stringify(searchQuery));
    localStorage.setItem('piutang_statusFilter', JSON.stringify(statusFilter));
    localStorage.setItem('piutang_selectedCustIdx', JSON.stringify(selectedCustIdx));
    localStorage.setItem('piutang_selectedInvoiceIdx', JSON.stringify(selectedInvoiceIdx));
    localStorage.setItem('piutang_showPaymentModal', JSON.stringify(showPaymentModal));
    localStorage.setItem('piutang_paymentMode', JSON.stringify(paymentMode));
    localStorage.setItem('piutang_paymentAmount', JSON.stringify(paymentAmount));
    localStorage.setItem('piutang_manualAmounts', JSON.stringify(manualAmounts));
    localStorage.setItem('piutang_paymentMethod', JSON.stringify(paymentMethod));
    localStorage.setItem('piutang_paymentNote', JSON.stringify(paymentNote));
    localStorage.setItem('piutang_paymentDate', JSON.stringify(paymentDate));
    localStorage.setItem('piutang_syncNotaOtomatis', JSON.stringify(syncNotaOtomatis));
    localStorage.setItem('piutang_showDetailModal', JSON.stringify(showDetailModal));
    localStorage.setItem('piutang_detailInvoiceId', JSON.stringify(detailInvoiceId));
    localStorage.setItem('piutang_showReceiptModal', JSON.stringify(showReceiptModal));
    localStorage.setItem('piutang_receiptSession', JSON.stringify(receiptSession));
    localStorage.setItem('piutang_showBillingPrintModal', JSON.stringify(showBillingPrintModal));
    localStorage.setItem('piutang_printBillingGroup', JSON.stringify(printBillingGroup));
    localStorage.setItem('piutang_isConfirmed', JSON.stringify(isConfirmed));
  }, [
    searchQuery, statusFilter, selectedCustIdx, selectedInvoiceIdx,
    showPaymentModal, paymentMode, paymentAmount, manualAmounts, paymentMethod,
    paymentNote, paymentDate, syncNotaOtomatis, showDetailModal, detailInvoiceId,
    showReceiptModal, receiptSession, showBillingPrintModal, printBillingGroup, isConfirmed
  ]);
  ...
  return (
    <div className="space-y-6">
      ...
      {toast && (
        <ModalPortal>
          <div className={`fixed bottom-4 right-4 z-[100000] px-4 py-2.5 rounded-lg shadow-lg flex items-center gap-3 animate-fade-in bg-white border ${toast.type === 'success' ? 'border-blue-600 text-blue-600' : 'border-danger-600 text-danger-600'
            }`}>
